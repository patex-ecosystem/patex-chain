...
func readGenesisConfig(ctx *cli.Context) *core.Genesis {
	// Patex just return already stored genesis config
	if ctx.IsSet(utils.PatexSepoliaFlag.Name) {
		return config.PatexSepoliaGenesisConfig
	}
	// Patex just return already stored genesis config
	if ctx.IsSet(utils.PatexMainnetFlag.Name) {
		return config.PatexMainnetGenesisConfig
	}
...
func makeFullNode(ctx *cli.Context) (*node.Node, ethapi.Backend) {
	...
	if ctx.IsSet(utils.OverridePatexBedrock.Name) {
		cfg.Eth.OverridePatexBedrock = flags.GlobalBig(ctx, utils.OverridePatexBedrock.Name)
	}
	if ctx.IsSet(utils.OverridePatexRegolith.Name) {
		v := ctx.Uint64(utils.OverridePatexRegolith.Name)
		cfg.Eth.OverridePatexRegolith = &v
	}
	if ctx.IsSet(utils.OverridePatex.Name) {
		override := ctx.Bool(utils.OverridePatex.Name)
		cfg.Eth.OverridePatex = &override
	}
