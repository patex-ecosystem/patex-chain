...
type ChainOverrides struct {
	...
	// patex
	OverridePatexBedrock  *big.Int
	OverridePatexRegolith *uint64
	OverridePatex         *bool
}
...
func SetupGenesisBlockWithOverride(db ethdb.Database, triedb *trie.Database, genesis *Genesis, overrides *ChainOverrides) (*params.ChainConfig, common.Hash, error) {
	...
	applyOverrides := func(config *params.ChainConfig) {
		if config != nil {
			if config.IsPatex() && config.ChainID != nil && config.ChainID.Cmp(params.PatexGoerliChainId) == 0 {
				// Apply Patex Goerli regolith time
				config.RegolithTime = &params.PatexGoerliRegolithTime
			}
			if config.IsPatex() && config.ChainID != nil && config.ChainID.Cmp(params.BaseGoerliChainId) == 0 {
			...
			if overrides != nil && overrides.OverridePatexBedrock != nil {
				config.BedrockBlock = overrides.OverridePatexBedrock
			}
			if overrides != nil && overrides.OverridePatexRegolith != nil {
				config.RegolithTime = overrides.OverridePatexRegolith
			}
			if overrides != nil && overrides.OverridePatex != nil {
				if *overrides.OverridePatex {
					config.Patex = &params.PatexConfig{
