...
type receiptMarshaling struct {
	...
	// Patex: extend receipts with their L1 price (if a rollup tx)
...
// LegacyPatexStoredReceiptRLP is the pre bedrock storage encoding of a
...
// entries.
type LegacyPatexStoredReceiptRLP struct {
	PostStateOrStatus []byte
	CumulativeGasUsed uint64
	Logs              []*LogForStorage
	L1GasUsed         *big.Int
...
func (r *ReceiptForStorage) DecodeRLP(s *rlp.Stream) error {
	...
	// First try to decode the latest receipt database format, try the pre-bedrock Patex legacy format otherwise.
	if err := decodeStoredReceiptRLP(r, blob); err == nil {
		return nil
	}
	return decodeLegacyPatexReceiptRLP(r, blob)
}

func decodeLegacyPatexReceiptRLP(r *ReceiptForStorage, blob []byte) error {
	var stored LegacyPatexStoredReceiptRLP
...
func (rs Receipts) DeriveFields(config *params.ChainConfig, hash common.Hash, number uint64, time uint64, baseFee *big.Int, txs Transactions) error {
	...
	if config.Patex != nil && len(txs) >= 2 { // need at least an info tx and a non-info tx
