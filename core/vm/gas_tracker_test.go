...
func assertEtherBalance(t *testing.T, state StateDB, address common.Address, desiredBalance uint64) {
	...
	if address.String() == params.PatexGasAddress.String() {
...
func TestUnsetContract(t *testing.T) {
	...
	patexEtherBalance := db.GetBalance(params.PatexBaseFeeRecipient)
	if patexEtherBalance.Cmp(new(big.Int).SetUint64(5)) != 0 {
		fmt.Println(patexEtherBalance)
...
func TestMultipleUse(t *testing.T) {
	...
	patexEtherBalance := db.GetBalance(params.PatexBaseFeeRecipient)
	if patexEtherBalance.Cmp(new(big.Int).SetUint64(1)) != 0 {
		t.Fatalf("patex ether balance incorrect")
	}
...
func TestMultipleContracts(t *testing.T) {
	...
	assertEtherBalance(t, db, params.PatexGasAddress, 5)
...
func TestRefundContractWithoutPatexGas(t *testing.T) {
	db, _ := state.New(types.EmptyRootHash, state.NewDatabase(rawdb.NewMemoryDatabase()), nil)
	effectiveTip := big.NewInt(1)
	setGasMode(db, getAddr(1), common.Big1)
	setGasMode(db, getAddr(2), common.Big1)
	...
	assertEtherBalance(t, db, params.PatexGasAddress, 1)
	assertEtherBalance(t, db, getAddr(1), 4)
	assertEtherBalance(t, db, getAddr(2), 4)
}

func TestRefundContractWithPatexGas(t *testing.T) {
	db, _ := state.New(types.EmptyRootHash, state.NewDatabase(rawdb.NewMemoryDatabase()), nil)
	effectiveTip := big.NewInt(1)
	setGasMode(db, getAddr(1), common.Big1)
	setGasMode(db, getAddr(2), common.Big1)
	...
	assertEtherBalance(t, db, params.PatexGasAddress, 6)
	assertEtherBalance(t, db, getAddr(1), 4)
	assertEtherBalance(t, db, getAddr(2), 4)
	assertEtherBalance(t, db, getAddr(3), 0)
}
...
func TestGasBalanceInPredeploy(t *testing.T) {
	...
	patexEtherBalance := db.GetBalance(params.PatexBaseFeeRecipient)
	if patexEtherBalance.Cmp(big.NewInt(24)) != 0 {
		t.Fatalf("Balance is not correct. Expected: %v, Got: %v", big.NewInt(24), patexEtherBalance)
	}
}
