...
var (
	...
	PatexGoerliRegolithTime = uint64(1679079600)
...
var (
	...
	// This is an Patex chain config with bedrock starting a block 5, introduced for historical endpoint testing, largely based on the clique config
	PatexTestConfig = func() *ChainConfig {
		conf := *AllCliqueProtocolChanges // copy the config
		conf.Clique = nil
		conf.BedrockBlock = big.NewInt(5)
		conf.Patex = &PatexConfig{EIP1559Elasticity: 50, EIP1559Denominator: 10}
		return &conf
	}()
)
...
type ChainConfig struct {
	...
	BedrockBlock *big.Int `json:"bedrockBlock,omitempty"` // Bedrock switch block (nil = no fork, 0 = already on patex bedrock)
	RegolithTime *uint64  `json:"regolithTime,omitempty"` // Regolith switch time (nil = no fork, 0 = already on patex regolith)
	...
	// Patex config, nil if not active
	Patex *PatexConfig `json:"patex,omitempty"`
}
...
// PatexConfig is the patex config.
type PatexConfig struct {
	EIP1559Elasticity  uint64 `json:"eip1559Elasticity"`
	EIP1559Denominator uint64 `json:"eip1559Denominator"`
}

// String implements the stringer interface, returning the patex fee config details.
func (o *PatexConfig) String() string {
	return "patex"
}
...
// IsPatex returns whether the node is an patex node or not.
func (c *ChainConfig) IsPatex() bool {
	return c.Patex != nil
}

// IsPatexBedrock returns true iff this is an patex node & bedrock is active
func (c *ChainConfig) IsPatexBedrock(num *big.Int) bool {
	return c.IsPatex() && c.IsBedrock(num)
}

func (c *ChainConfig) IsPatexRegolith(time uint64) bool {
	return c.IsPatex() && c.IsRegolith(time)
}

// IsPatexPreBedrock returns true iff this is an patex node & bedrock is not yet active
func (c *ChainConfig) IsPatexPreBedrock(num *big.Int) bool {
	return c.IsPatex() && !c.IsBedrock(num)
}
...
func (c *ChainConfig) BaseFeeChangeDenominator() uint64 {
	if c.Patex != nil {
		return c.Patex.EIP1559Denominator
	}
...
func (c *ChainConfig) ElasticityMultiplier() uint64 {
	if c.Patex != nil {
		return c.Patex.EIP1559Elasticity
	}
...
type Rules struct {
	...
	IsPatexBedrock, IsPatexRegolith                         bool
}
...
func (c *ChainConfig) Rules(num *big.Int, isMerge bool, timestamp uint64) Rules {
	...
	return Rules{
		...
		// Patex
		IsPatexBedrock:  c.IsPatexBedrock(num),
		IsPatexRegolith: c.IsPatexRegolith(timestamp),
	}
}
