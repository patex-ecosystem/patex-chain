...
func newOpTestBackend(t *testing.T, txs []testTxData) *opTestBackend {
	...
	// only the most recent block is considered for patex priority fee suggestions, so this is
...
func TestSuggestPatexPriorityFee(t *testing.T) {
	...
	for i, c := range cases {
		...
		got := oracle.SuggestPatexPriorityFee(context.Background(), backend.block.Header(), backend.block.Hash())
		if got.Cmp(c.want) != 0 {
