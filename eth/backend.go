...
func New(stack *node.Node, config *ethconfig.Config) (*Ethereum, error) {
	...
	if config.OverridePatexBedrock != nil {
		overrides.OverridePatexBedrock = config.OverridePatexBedrock
	}
	if config.OverridePatexRegolith != nil {
		overrides.OverridePatexRegolith = config.OverridePatexRegolith
	}
	if config.OverridePatex != nil {
		overrides.OverridePatex = config.OverridePatex
	}
	eth.blockchain, err = core.NewBlockChain(chainDb, cacheConfig, config.Genesis, &overrides, eth.engine, vmConfig, eth.shouldPreserve, &config.TxLookupLimit)
	if err != nil {
		return nil, err
	}
	if chainConfig := eth.blockchain.Config(); chainConfig.Patex != nil { // config.Genesis.Config.ChainID cannot be used because it's based on CLI flags only, thus default to mainnet L1
		config.NetworkId = chainConfig.ChainID.Uint64() // patex defaults eth network ID to chain ID
		eth.networkID = config.NetworkId
	}
	log.Info("Initialising Ethereum protocol", "network", config.NetworkId, "dbversion", dbVer)

	if eth.blockchain.Config().Patex != nil { // Patex Bedrock depends on Merge functionality
