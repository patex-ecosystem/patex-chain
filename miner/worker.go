...
func (w *worker) pending() (*types.Block, *state.StateDB) {
	if w.chainConfig.Patex != nil && !w.config.RollupComputePendingBlock {
...
func (w *worker) makeEnv(parent *types.Header, header *types.Header, coinbase common.Address) (*environment, error) {
	...
	if err != nil && w.chainConfig.Patex != nil { // Allow the miner to reorg its own chain arbitrarily deep
...
func (w *worker) prepareWork(genParams *generateParams) (*environment, error) {
	...
	// Set the extra field.
	if len(w.extra) != 0 && w.chainConfig.Patex == nil { // Patex chains must not set any extra data.
	...
	} else if w.chain.Config().Patex != nil && w.config.GasCeil != 0 {
		// configure the gas limit of pending blocks with the miner gas limit config when using patex
